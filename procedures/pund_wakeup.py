...
class PUNDWakeUpProcedure(MeasurementProcedure):
    ...
    def _build_triangular_pulse_vectors(self, frequency, vmax, signs):
        ...
        for sign in signs:
            vectors += [
                (pulse_width, self._positive_level(vmax) if sign > 0 else self._negative_level(vmax)),
    ...
    def _build_read_vectors(self, vmax):
        ...
        if self.read_pulse_type == 'pund':
            return self._build_triangular_pulse_vectors(self.read_pulse_freq, vmax, self.PUND_SIGNS)

        vectors = []
        vectors.extend(self._build_pn_vectors(vmax))
        self._append_idle_hold(vectors, 1.0 / self.read_pulse_freq)
        vectors.extend(self._build_triangular_pulse_vectors(self.read_pulse_freq, vmax, self.PUND_SIGNS))
        return vectors
    ...
    def _read_phase_spans(self, vmax):
        ...
        pund_duration = sum(
            dt for dt, _ in self._build_triangular_pulse_vectors(self.read_pulse_freq, vmax, self.PUND_SIGNS)
        )
    ...
    def measure(self, device):
        ...
        sample_points = max(1, int(round(max_read_duration / sample_interval)))
            ...
            while True:
                ...
                if available > plotted_count:
                    ...
                    for sample_index in range(plotted_count, available):
                        ...
                        phase = self._phase_for_time(read_time_s, phase_spans_by_vmax[read_idx])
                        if phase is None:
                            continue
            ...
            for read_idx, vmax in enumerate(self.vmax_values):
                ...
                for i in range(sample_points):
                    ...
                    if row_idx < len(data_ch1) and row_idx < len(data_ch2):
                        ...
                        timestamp = t_v if t_v is not None else t_i
                        all_rows.append([vmax, timestamp, voltage, current])
