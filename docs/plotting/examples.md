...
runner.plot.configure(title=f"R-V Sweep - {device.name}", plots=[
    PlotDef("rv", xlabel="Voltage (V)", ylabels=("Current (A)",),
            elements=[
                Curve("I_V", legend_label="I(V)"),
            ]),
])

for v, i in zip(voltages, currents):
    runner.plot.append_point("I_V", v, i)
...
runner.plot.configure(title=f"4-Term IV - {device.name}", plots=[
    PlotDef("iv", xlabel="Current (A)", ylabels=("Voltage (V)",),
            elements=[
                Curve("V_I", mode="scatter", marker="x", color="C0",
                      legend_label="V(I)"),
                LinearFit("V_I", color="C1",
                          legend_label_template="R = {slope:.4g} Ohm  (R^2 = {r_squared:.4f})"),
            ]),
])
...
for idx in range(num_points):
    runner.check_stop()
    # ... perform measurement ...
    runner.plot.append_point("V_I", current, voltage)

# read fit result for data output (same data, same algorithm as viewer)
...
for i, freq in enumerate(frequencies):
    ...
    tag = freq_label(freq)
    elements.append(Curve(f"Cp_{tag}", color=color,
                          legend_label=f"Cp @ {tag}"))
    elements.append(Curve(f"Td_{tag}", color=color, line_style="dash",
                          yaxis=1, legend_label=f"Tan(d) @ {tag}"))
...
# during sweep
runner.plot.append_point(f"Cp_{tag}", bias, capacitance)
runner.plot.append_point(f"Td_{tag}", bias, tan_delta)
...
runner.plot.append_many(f"V_{cyc}", times, voltages)
runner.plot.append_many(f"I_{cyc}", times, currents_ua)
...
runner.plot.append_batch({
    "I1": list(zip(bucket_times, bucket_i1)),
    "I2": list(zip(bucket_times, bucket_i2)),
})
...
runner.plot.configure(title="Multi-panel Analysis", plots=[
    ...
    PlotDef("cv", row=0, col=1,
            ...
            xlink="iv",
            elements=[Curve("C_V", legend_label="C(V)")]),
    PlotDef("endurance", row=1, col=0, colspan=2,
            xlabel="Cycle", ylabels=("2Pr (uC/cm2)",),
            elements=[
                Curve("Pr", mode="scatter", marker="o", legend_label="2Pr"),
                LinearFit("Pr", color="C1",
                          legend_label_template="trend: {slope:.2g}/cycle"),
            ]),
])
```
